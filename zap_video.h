...
/* ---------------- canonical Huffman for the token streams (order-0, max code length 11, LSB-first bits)
...
/* decodes exactly raw symbols; 0 ok, -1 corrupt */
static inline int zap__hdec(const uint8_t *in, size_t n, uint8_t *out, size_t raw) {
    uint8_t len[256];
    uint16_t code[256], table[1 << ZAP__HMAX];
    if (n < 128) return -1;
    uint32_t kraft = 0;
    ...
    const uint8_t *p = in + 128, *e = in + n;
    uint64_t acc = 0;
    int nb = 0;
    for (size_t i = 0; i < raw; i++) {
        while (nb <= 56 && p < e) { acc |= (uint64_t)*p++ << nb; nb += 8; }
        int t = table[acc & ((1u << ZAP__HMAX) - 1)], l = t >> 8;
        if (!l || l > nb) return -1; /* unused code, or ran past the end */
        out[i] = (uint8_t)t; acc >>= l; nb -= l;
    }
    return 0;
}
